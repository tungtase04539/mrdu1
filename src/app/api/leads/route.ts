...
import { createLead, hasSupabaseWriteConfig, type Lead } from "@/lib/supabase-rest";

export async function POST(request: Request) {
  if (!hasSupabaseWriteConfig()) {
    return NextResponse.json({ error: "Form liên hệ chưa được cấu hình lưu dữ liệu. Vui lòng gọi hotline hoặc Zalo." }, { status: 503 });
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: isMissingTable ? "Form liên hệ chưa có bảng leads trong Supabase. Vui lòng gọi hotline hoặc Zalo." : isBadJson ? message : "Không gửi được yêu cầu tư vấn. Vui lòng gọi hotline hoặc Zalo." },
